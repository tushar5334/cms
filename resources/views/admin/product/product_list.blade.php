    ...
    <div class="col-12">
      <div class="card card-outline card-primary">
        <div class="card-header">
          <h3 class="card-title float-none float-sm-left mb-3">Products List</h3>
        ...
        <div class="card-body">
          ...
          <div class="table-overflow">
            <table id="product_datatable" class="table table-bordered table-striped display responsive"
              style="width:100%">
              <thead>
                <tr>
                  <th></th>
      ...
      var dataTable = $('#product_datatable').DataTable({
          ...
          columns: [
            ...
            },
            {
              ...
              searchable: false,
              render: function(data, type, row, meta) { 
                var edit_url = '{{ route("admin.products.edit", ":product_id") }}';
                edit_url = edit_url.replace(':product_id', row.product_id);
                var str = "";
                str += '<div class="btn-group">';
                 if(row.status == 1){
                  str += '<a title="Status" id="'+row.product_id+'" class="btn btn-success status_active" href="javascript:;" data-id="'+row.product_id+'"><i class="fas fa-check-circle"></i></a>';
                }else{
                  str += '<a title="Status" id="'+row.product_id+'" class="btn btn-default status_active" href="javascript:;" data-id="'+row.product_id+'"><i class="fas fa-ban"></i></a>';
                }
                str+= '<a title="Edit" id="'+row.product_id+'" class="btn btn-warning edit_icon icon user-edit" href="'+edit_url+'"><i class="fas fa-edit"></i></a>';
                str += '<a title="Delete" id="'+row.product_id+'" class="btn btn-danger delete_icon icon delete_record" data-tooltip="Delete" href="javascript:;" data-id="'+row.product_id+'" data-toggle="modal" data-target="#modal-sm-'+row.product_id+'"><i class="fas fa-trash"></i></a>'
                str += '</div>';
                return str;
              }
            }
          ...
          fnDrawCallback: function( oSettings ) {
            // Delete Record
            $('#product_datatable tbody').on( 'click', 'tr td .status_active', function (e) {
            let statusId = $(this).attr("data-id");
            let url = "{{ route('admin.post.products.change-status') }}";
            self.confirmStatus(statusId, url, dataTable);
            });
            $('#product_datatable tbody').on( 'click', 'tr td .delete_record', function (e) {
            let deletedId = $(this).attr("data-id");
            let url = '{{ route("admin.products.destroy", ":product_id") }}';
            url = url.replace(':product_id', deletedId);
           ...
           fnInitComplete: function(oSettings, json) {
            $('.deleteMultiple').on('click',function(e){
              ...
              if(selectedIds.length > 0)
              {
                ...
                selectedIds = [];
                let url = "{{ route('admin.post.products.multiple-delete') }}";
            ...
            $('.activeMultiple').on('click',function(){
              ...
              if(selectedIds.length > 0)
              {
                ...
                selectedIds = [];
                let url = "{{ route('admin.post.products.multiple-change-status') }}";
