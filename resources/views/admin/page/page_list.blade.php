    ...
    <div class="col-12">
      <div class="card card-outline card-primary">
        <div class="card-header">
          <h3 class="card-title float-none float-sm-left mb-3">Page List</h3>
        ...
        <div class="card-body">
          ...
          <div class="table-overflow">
            <table id="cms_datatable" class="table table-bordered table-striped display responsive" style="width:100%">
              <thead>
                <tr>
                  <th></th>
                  <th>#</th>
...
<script>
  $(function () {
     $('.datepicker').datepicker({
      ...
      var startDate = "";
      var endDate = "";
      var dataTable = $('#cms_datatable').DataTable({
          processing: false,
          serverSide: true,          
          info: true,
          lengthChange: true,
          ...
          columns: [
              ...
              render: function(data, type, row, meta) {
               ...
               if(data){
                if(data.length > Char) {
                  ...
                  str +=  '<div id="showrequest_'+row.page_id+'">';
                  str +=  '<a class="font-weight-light" title="'+cleanStr+'"> '+SmallText + '</a>';
                  str +=  "<a id=\"toggleButton\" class='text-primary btn-sm toggleButton' data-id="+row.page_id+" data-content="+cleanStr+" href=\"javascript:void(0);\">... more</a>";
                  str +=  '</div>';

                  str +=  '<div id="hiderequest_'+row.page_id+'" style="display:none">';
                  str +=  '<a style="word-break:break-all" class="font-weight-light" title="'+cleanStr+'"> '+cleanStr + '</a>';
                  str +=  "<a id=\"toggleButton\" class='text-primary btn-sm toggleButton' data-id="+row.page_id+" data-content="+cleanStr+" href=\"javascript:void(0);\">less</a>";
                  str +=  '</div>';
            ...
            },
            {
              ...
              searchable: false,
              render: function(data, type, row, meta) { 
                var edit_url = '{{ route("admin.pages.edit", ":page_id") }}';
                edit_url = edit_url.replace(':page_id', row.page_id);
                var str = "";
                str += '<div class="btn-group">';
                 if(row.status == 1){
                  str += '<a title="Status" id="'+row.page_id+'" class="btn btn-success status_active" href="javascript:;" data-id="'+row.page_id+'"><i class="fas fa-check-circle"></i></a>';
                }else{
                  str += '<a title="Status" id="'+row.page_id+'" class="btn btn-default status_active" href="javascript:;" data-id="'+row.page_id+'"><i class="fas fa-ban"></i></a>';
                }
                str+= '<a title="Edit" id="'+row.page_id+'" class="btn btn-warning edit_icon icon user-edit" href="'+edit_url+'"><i class="fas fa-edit"></i></a>';
                str += '<a title="Delete" id="'+row.page_id+'" class="btn btn-danger delete_icon icon delete_record" data-tooltip="Delete" href="javascript:;" data-id="'+row.page_id+'" data-toggle="modal" data-target="#modal-sm-'+row.page_id+'"><i class="fas fa-trash"></i></a>'
                str += '</div>';
                return str;
              }
            }
          ...
          fnDrawCallback: function( oSettings ) {
            // Delete Record
            $('#cms_datatable tbody').on( 'click', 'tr td .status_active', function (e) {
            let statusId = $(this).attr("data-id");
            alert(statusId);
            let url = "{{ route('admin.post.pages.change-status') }}";
            self.confirmStatus(statusId, url, dataTable);
            });

            $('#cms_datatable tbody').on( 'click', 'tr td .delete_record', function (e) {
            let deletedId = $(this).attr("data-id");
            let url = '{{ route("admin.pages.destroy", ":page_id") }}';
            url = url.replace(':page_id', deletedId);
           ...
           fnInitComplete: function(oSettings, json) {
            $('.deleteMultiple').on('click',function(e){
              ...
              if(selectedIds.length > 0)
              {
                ...
                selectedIds = [];
                let url = "{{ route('admin.post.pages.multiple-delete') }}";
            ...
            $('.activeMultiple').on('click',function(){
              ...
              if(selectedIds.length > 0)
              {
                ...
                selectedIds = [];
                let url = "{{ route('admin.post.pages.multiple-change-status') }}";
